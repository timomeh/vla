...
Colocate [Actions](/reference/classes/action) in your presentation layer (next to your UI components) in a separate file. Create a `data.ts` file for each group of files, which belong together. Create multiple actions in this `data.ts`.
